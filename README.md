...
v0.0.7 - Попытка использования ультразвукового эхолота. На микросекундных задержках работает нестабильно. 
Надо пробовать вариант с прерываниями, либо вообще использовать Arduino Nano, 
там нет многозадачности, работает стабильно.  
Примечание: при использовании делителя на входе ESP32 при работе 
с 5В выходом Arduino Nano подтягивающий резистор не надо включать - портится сигнал.
...
Работающая версия на кухне.

### V4
ESP32H2 + ультразвуковой датчик (Arduino Nano + HC-SR04).  

### V5
ESP32H2 + BMP280
Датчик температуры и давления на балкон. Работающая версия.     

### V6
ESP32H2 + PAJ7620U2
Датчик жестов и приближения.
